...
impl<'a> HdrOutput<'a> {
    ...
    pub fn y_row(&self, row: usize) -> &'a [u16] {
        assert!(row < self.height as usize, "row out of range");
        let start = row * (self.y_stride as usize / 2);
        &self.y()[start..start + (self.width) as usize]
    }

    /// Active samples only; panics if row is outside the plane.
    pub fn u_row(&self, row: usize) -> &'a [u16] {
        assert!(row < self.chroma_h as usize, "row out of range");
        let start = row * (self.uv_stride as usize / 2);
        &self.u()[start..start + (self.width / 2) as usize]
    }

    /// Active samples only; panics if row is outside the plane.
    pub fn v_row(&self, row: usize) -> &'a [u16] {
        assert!(row < self.chroma_h as usize, "row out of range");
        let start = row * (self.uv_stride as usize / 2);
    ...
    pub fn to_owned(&self) -> crate::OwnedOutput<u16> {
        crate::OwnedOutput {
