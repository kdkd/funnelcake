...
int fused_hdr_init(fused_hdr_ctx_t *ctx)
{
    ...
    /* ------------------------------------------------------------------ */
    /* 3. Validate requested_flags                                          */
    /* ------------------------------------------------------------------ */

    uint32_t req = ctx->requested_flags;

    if (req == 0 && !ctx->tonemap_1x) {
        fused_log(&ctx->log_errors, FUSED_LOG_ERROR,
            "funnelcake-hdr: requested_flags is zero\n");
        return FUSED_ERR_INVALID_FLAGS;
    }
    ...
    if (achieved_any == 0 && !ctx->tonemap_1x) {
        fused_hdr_free(ctx);
        free(state);
        ctx->_internal = NULL;
        fused_log(&ctx->log_errors, FUSED_LOG_ERROR,
                   ...
                   const uint16_t *src_v)
{
    ...
    if (((uintptr_t)src_y & 31) || ((uintptr_t)src_u & 31) ||
        ...
        if (state->params.family == FUSED_FAMILY_THIRDS)
            fused_kernel_thirds_hdr_scalar(&state->params, src_y, src_u, src_v);
        else
            fused_kernel_pow2_hdr_scalar(&state->params, src_y, src_u, src_v);
    } else {
        /* ------------------------------------------------------------------ */
